...
const ExistingChartSelectDialog = props => {
  ...
  return (
    ...
    >
      <DialogTitle>
        Select a Chart from your History
      ...
      <DialogContent align="center">
        <FormControl
          style={{ minWidth: "8em", marginBottom: "2em", width: "85%" }}
        >
          <InputLabel>Existing Charts</InputLabel>
          <Select style={{ width: "100%" }} onChange={handleChange}>
            {graphData.map((data, idx) => (
              <MenuItem value={data} key={idx}>
                {data.selectedChart.selectedChart} - x:
                {data.xAxisColumn.xAxisColumn}, y:{" "}
                {data.yAxisColumn.yAxisColumn}
              </MenuItem>
            ))}
          </Select>
        </FormControl>
